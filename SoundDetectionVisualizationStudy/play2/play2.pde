...
void draw()
{
  ...
  for(int i = 0; i < beat.detectSize(); ++i)
  {
    ...
    if ( beat.isOnset(i) )
    {
      ...
      ellipse( i*rectW, 0, random(5), random(200));
      fill(random(255),0,random(155));
       ellipse( i*rectW, height*0.9, random(5), random(200));
    }
  }
  ...
  //kickSize = constrain(kickSize * 0.95, 16, 128);
  //snareSize = constrain(snareSize * 0.95, 16, 128);
  //hatSize = constrain(hatSize * 0.95, 16, 128);
 ...
 beat.detect(song.mix);
  ...
  fill(205, 15, 205, a);
  noStroke();
  if ( beat.isKick() ) eRadius = 355;
  if (beat.isSnare()) eRadius=180;
  if (beat.isHat()) eRadius=50;
  ellipse(width/2, height/2, eRadius, eRadius);
  eRadius *= 0.95;
  if ( eRadius < 20 ) eRadius = 30;
