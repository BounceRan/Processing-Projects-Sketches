...
void draw()
{
  ...
  fill(15, 0, 15, 10);
   //box(width+eRadius, width+eRadius,width+eradius  );
      //noStroke();
  beat.detect(in.mix);
  float a = map(eRadius, 20, 80, 60, 255);
  fill(random(155), 55, 155, 30);
  if ( beat.isOnset() ) eRadius = 250;
  if(beat.isHat())cRadius=200;
 // rectMode(CENTER);
 
  sphere(eRadius-50);
  eRadius *= 0.95;
  if ( eRadius < 50 ) eRadius = 50;
  cRadius*=0.84;
  if ( cRadius < 120 ) cRadius = 10;
